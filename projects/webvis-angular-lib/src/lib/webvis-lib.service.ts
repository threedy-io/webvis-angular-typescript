...
export class WebvisLibService {
  ...
  private selectionChangedSubjects = new Map<string | undefined, Subject<number>>();
  private isProcessingSubjects = new Map<string | undefined, BehaviorSubject<boolean>>();
  ...
  public registerListenersForContext(ctxName?: string)
  {
    this.getWebvisContext(ctxName).then(ctx =>
      ctx.registerListener(
        [webvis.EventType.SELECTION_CHANGED, webvis.EventType.PROGRESS_CHANGED,webvis.EventType.VIEWER_CREATED], 
        event => this.ngZone.run(() => this.handleWebvisEvent(event, ctxName))
      )
    );
  }
  
  async addNodeFromUrl(dataURI: string, ctxName?: string, enabled = true): Promise<number> {
    const ctx = await this.getWebvisContext(ctxName);
    const node = ctx.add(dataURI);

    // Check NodeInfoState, then Enable, then resolve
    return ctx.getProperty(node, "infoState")
      .then( state => {
        if (state !== webvis.NodeInfoState.DEFAULT)
          throw new Error("Wrong NodeInfoState");
      })
      .then( () => ctx.setProperty(node, "enabled", enabled))
  ...
  async getWebvisContext(ctxName?: string): Promise<webvis.ContextAPI> {
    return firstValueFrom(this.webvisLoaded$.pipe(
      filter(val => !!val),
  ...
  async createContext( ctxName?: string): Promise<webvis.ContextAPI> {
    return (<any>window).webvis.requestContext(ctxName);
  }

  handleWebvisEvent(event: webvis.WebVisEvent, ctxName? : string) {
    switch (event.type) {
      case webvis.EventType.SELECTION_CHANGED:
        this.getSelectionChangeSubject(ctxName).next((event as webvis.SelectionChangedEvent).targetNodeID);
        break;
      case webvis.EventType.PROGRESS_CHANGED:
        this.getProcessingSubject(ctxName).next( ( event as webvis.ProgressChangedEvent).progressState.downloadProgress < 100);
        break;
      default:
        break;
    }
